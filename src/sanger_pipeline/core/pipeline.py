...
class SangerPipelineRefactored:
    ...
    def __init__(
        ...
        logger.info(
            f"Quality settings: min_quality={self.min_quality}, min_sequence_length={self.min_sequence_length}"
        )
    ...
    def _initialize_components(self) -> None:
        ...
        self.ab1_converter = AB1Converter(
            min_quality=self.min_quality, min_sequence_length=self.min_sequence_length
        )
    ...
    def _initialize_step_handlers(self) -> None:
        ...
        self.damage_analysis_step = DamageAnalysisStep(
            self.damage_analyzer, self.config
        )
    ...
    def get_summary(self) -> Dict:
        ...
        summary = {
            ...
            "filtered_files": len(
                list(self.directories["filtered"].glob("*_filtered.fasta"))
            ),
            "consensus_files": len(
                list(self.directories["consensus"].glob("*_consensus.fasta"))
            ),
        ...
        if damage_dir.exists():
            summary["damage_results"] = len(
                list(damage_dir.glob("*_damage_results.json"))
            )
